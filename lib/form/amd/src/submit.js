...
export const init = (elementId) => {
    ...
    document.addEventListener(FormEvents.eventTypes.uploadStarted, e => {
    ...
    document.addEventListener(FormEvents.eventTypes.uploadCompleted, e => {
        window.console.log(e.target); // This will be the element/section where the file is uploaded to.

        // Only enable if all files are uploaded completely.
        if (FormEvents.uploadCompleted()) {
            button.disabled = false;
        }
    });
